...
from strategy_bb import BollingerBandStrategy
import time
import configparser
...
def strategy_logic():
    while True:
        for symbol in symbols:
            ...
            latest_candle_4h = broker.fetch_latest_candle(epic=symbol, resolution="HOUR_4")
            latest_candle_1h = broker.fetch_latest_candle(epic=symbol, resolution="HOUR")
            
            if latest_candle is not None:
                # Update strategy with the latest candle
                #strategies_tiple_ema[symbol].update_ohlc(latest_candle)
                strategies_bb[symbol].update_ohlc(latest_candle_4h, latest_candle_1h)
                # Generate buy/sell signal
                #signal = strategies_tiple_ema[symbol].generate_signal()
                signal_bb = strategies_bb[symbol].generate_signal()
                ...
                if signal_bb == "BUY":
                    sl, tp = strategies_bb[symbol].set_targets_and_stop_loss(signal="BUY")
                    broker.send_order(symbol=symbol, side="BUY", size=0.0, tp=tp, sl=sl)
                elif signal_bb == "SELL":
                    sl, tp = strategies_bb[symbol].set_targets_and_stop_loss(signal="SELL")
                    broker.send_order(symbol=symbol, side="SELL", size=0.0, tp=tp, sl=sl)
...
def initialize_strategies():
    for symbol in symbols:
        ...
        ohlc_data_4h = broker.get_historical_data_in_chunks(epic=symbol, resolution="HOUR_4", start_date="2024-09-15T00:00:00", max=200)
        ohlc_data_1h = broker.get_historical_data_in_chunks(epic=symbol, resolution="HOUR", start_date="2024-09-15T00:00:00", max=200)
        #strategies_tiple_ema[symbol] = TripleMAMACDStrategy(ohlc_data)
        strategies_bb[symbol] = BollingerBandStrategy(ohlc_data_1h=ohlc_data_1h, ohlc_data_4h=ohlc_data_4h)

if __name__ == "__main__":
    # Define symbols to monitor
    symbols = ["BTCUSD", 
               #"ETHUSD", "EURUSD", "GBPUSD", "GOLD", "US100", "US30", "US500"
               ]
